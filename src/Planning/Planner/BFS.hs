...
instance (Domain d as, Problem p) => ExternalPlanner BFS d p as where
...
search dom p =
    do (_, as) <- bfs Set.empty $ trace' "queue " (Queue.fromList [(initialState p, [])])
       return $ reverse as
          ...
          bfs explored queue = trace (show (Set.size explored)) $ trace ("state size " ++ (show (maxLength explored))) $ do
            ...
            let actions' as = applicableActions as p s
                ...
                res | s `Set.member` explored = bfs explored queue'
                    | isSolved p s = Just n
                    | otherwise    = bfs (Set.insert s explored) $ trace' "enqueue " (foldl Queue.enqueue queue' uExpl)
             in res
